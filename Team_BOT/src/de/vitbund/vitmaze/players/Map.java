...
public class Map {
	...
	public Map(int xKord, int yKord) {
		this.xKord = xKord;
		this.yKord = yKord;
//
		map = new Feld2[yKord][xKord];

		for (int y = 0; y < this.yKord; y++) {
			for (int x = 0; x < this.xKord; x++) {
				map[y][x] = new Feld2(y, x, " ? ", null, false, false, 0);
...
//	}
	...
	public void sucheWeg(String position, String lastAction, int a, int b, int y, int x, int id) {
//		if (map[y][x].getStatus().equals("FINISH " + id + " 0")) {
//			System.out.println("finish");
//		}

		if (map[y - 1][x].isBetreten() == false && map[y - 1][x].getStatus().equals("FINISH " + id + " 0")) {
			System.out.println("go north");
		...
		else if (map[y - 1][x].getZaehlerBetreten() >= 1 && !map[y - 1][x].getStatus().equals("WALL") && map[y - 1][x].getyWert() != yCur.get(a) && map[y - 1][x].getxWert() != xCur.get(b)) {
			System.out.println("go north");
		} else if (map[y + 1][x].getZaehlerBetreten() >= 1 && !map[y + 1][x].getStatus().equals("WALL") && map[y + 1][x].getyWert() != yCur.get(a) && map[y + 1][x].getxWert() != xCur.get(b)) {
			System.out.println("go south");
		} else if (map[y][x - 1].getZaehlerBetreten() >= 1 && !map[y][x - 1].getStatus().equals("WALL") && map[y][x - 1].getyWert() != yCur.get(a) && map[y][x - 1].getxWert() != xCur.get(b)){
			System.out.println("go west");
		} else if (map[y][x + 1].getZaehlerBetreten() >= 1 && !map[y][x + 1].getStatus().equals("WALL") && map[y][x - 1].getyWert() != yCur.get(a) && map[y][x + 1].getxWert() != xCur.get(b)){
			System.out.println("go east");
		}
